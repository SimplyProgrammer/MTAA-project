...
export default {
	get(url: string, config?) {
		return axios.get(url, config).then(resp => resp.data).catch(_err) 
	},

	get_auth(url: string, config?) {
		return axios.get(url, _config(config)).then(resp => resp.data).catch(_err)
	},

	get_data(url: string, config?) {
		return axios.get(url, config).then(_data).catch(_err)
	},

	get_auth_data(url: string, config?) {
		return axios.get(url, _config(config)).then(_data).catch(_err)
	},
